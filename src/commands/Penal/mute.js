...
module.exports = {
  ...
  execute: async (client, message, args, embed, author, channel, guild) => {
    ...
    message.channel.send((`**${member}** **(${member.id})** kullanıcısı başarıyla **"${reason}"** sebebiyle susturuldu (Ceza Numarası: \`#${db.fetch(`ceza_${guild.id}`)}\`)`))
    ...
    setTimeout(() => {
      ...
      client.channels.cache.get(config.penals.mute.log).send(new Discord.MessageEmbed().setColor("RANDOM").setTimestamp().setDescription(`${member} kişisinin susturması bitti!`))}
    },);
